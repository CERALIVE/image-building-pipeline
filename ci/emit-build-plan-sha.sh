...
#   1. DRY_RUN=1 ./build <board> resolves the containerized mkosi plan and logs
...
# Resolve the repo root from this script's location (ci/…) so the invocation and
...
if [[ -z "${log}" ]]; then
  log="$(mktemp)"
  cleanup_log="${log}"
  if ! DRY_RUN=1 INSTALL_BOOT_BSP=0 "${REPO_ROOT}/build" "${board}" >"${log}" 2>&1; then
