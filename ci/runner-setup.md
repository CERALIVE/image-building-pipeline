...
> acceptance harnesses [`tests/realhw-smoke.sh`](../tests/realhw-smoke.sh) and
> [`tests/rauc-rollback.sh`](../tests/rauc-rollback.sh) (aggregated by
> [`tests/realhw-suite.sh`](../tests/realhw-suite.sh)). Regular offline CI stays
...
`mkosi/runtime/ceralive-ci-uart-bootstrap-public.pem`:
...
cmp \
  ...
  <(openssl pkey -pubin -in mkosi/runtime/ceralive-ci-uart-bootstrap-public.pem -pubout -outform DER)
```
