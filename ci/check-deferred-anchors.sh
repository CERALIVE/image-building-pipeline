#!/usr/bin/env bash
#
# ci/check-deferred-anchors.sh — CI gate against STALE anchors in DEFERRED.md.
#
# WHY: docs/DEFERRED.md indexes every deferred / hardware-gated item with
...
DEFERRED="$REPO_ROOT/docs/DEFERRED.md"
...
if (( fail > 0 )); then
  printf 'RESULT: STALE ANCHORS — reconcile docs/DEFERRED.md\n'
  exit 1
fi
