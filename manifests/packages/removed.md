...
| 35 | `rauc` | KEEP | **Must-NOT-Have list (RAUC pair).** The A/B OTA client. Driven by `rauc-hawkbit-updater` (the only automatic trigger) and by the manual `ceralive-update` script; NOT by CeraUI's `system.startUpdate()`, which is the apt package path. | all |
