...
| 25 | `libmbim-utils` | KEEP | KEYFACT "ModemManager 1.24 closure": stays in `shared.list` to resolve the Debian modem dependency tree, then the app layer UPGRADES it with the current `~ceralive.2` fork. Removing it breaks that dependency resolution. | all |
