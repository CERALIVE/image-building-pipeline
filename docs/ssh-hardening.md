...
Canonical sources live under `mkosi/runtime/`; they are installed (not
...
`tests/systemd-ordering-cycle.test.sh`, asserts BOTH zero cycles AND that each
...
dedicated guard is `tests/ssh-firstboot-privsep.test.sh`, which asserts the
