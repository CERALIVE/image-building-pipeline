...
| `srtla` | `.github/workflows/release.yml:148-227` builds both package arches in `debian:bookworm-slim`. | Fixed Bookworm container (`:152-155`). | No target-suite reader or equality assertion. | Yes for the historical Bookworm floor: `:210-219` rejects imports newer than `GLIBC_2.36`. | **Deferred gap** — the assertion is real but not derived from the Trixie target. |
...
| DSP-3 | `srtla` | Existing Bookworm/2.36 gate is not tied to the device target. | Container and `GLIBC_*` ceiling derive from one producer-owned target declaration. |
