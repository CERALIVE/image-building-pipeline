...
image-building-pipeline/dev-sync --all          # frontend + backend + native
image-building-pipeline/dev-sync --frontend     # UI only (no restart)
image-building-pipeline/dev-sync --backend      # backend binary only
image-building-pipeline/dev-sync --native       # srtla sysext
```
...
|------|-------------------|------------------|
| Validate the sync **plan** (routing, watch dirs, watcher cmd) | `DRY_RUN=1 image-building-pipeline/dev-sync --all` | watch + routing layer, with **no SSH and no watcher spawn** |
