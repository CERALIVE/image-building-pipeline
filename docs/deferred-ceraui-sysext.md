...
> via the appfs backend (`mkosi/app/build-ceraui-appfs.sh`). Nothing in this doc
...
A sysext build of this tree (`lib/app-layer/sysext.sh::build_app_layer` copies
...
- `mkosi/app/build-ceraui-appfs.sh` — current (appfs) packaging
- `lib/app-layer/{interface,sysext,appfs}.sh` — the 3-verb / 2-backend contract
- `mkosi/LAYER-MAP.md` §Layer 4 (app) and §libsrt — layer boundaries
