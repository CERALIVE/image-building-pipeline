...
used by `mkosi/platform/x86/install-x86-grub.sh` to stage the EFI System Partition
(see [`mkosi/platform/x86/README.md`](../mkosi/platform/x86/README.md) §2.0). `ovmf`
...
UEFI firmware in SPI flash), but the build host does, to run `tests/qemu-x86.sh`
...
- [ ] Target disk capacity recorded (must be **≥ 32 GB** — the manifest assumes a
      real NVMe/SSD; see `manifests/boards/x86-minipc.yaml` line 22-23).
...
see `manifests/families/x86_64.yaml`). If the mini-PC exposes a serial header or a
...
`tests/preflash-verify.sh`, that asserts exact GPT geometry, both bootloader
...
```bash
IMAGE_PATH="$(ls -t images/x86-minipc/*.raw | head -1)" \
  bash tests/qemu-x86.sh
```
...
`tests/qemu-x86.sh`) — no VA-API/QSV device exists in qemu, so it cannot validate
...
- [ ] `tests/qemu-x86.sh` reaches `multi-user.target` with `ceralive.service`
      at least `loaded`.
...
2. GRUB reads `grubenv` from the ESP (`ORDER`, `<slot>_OK`, `<slot>_TRY`) and selects
   `rootfs_a` or `rootfs_b` per [`mkosi/platform/x86/README.md`](../mkosi/platform/x86/README.md)
...
`rauc` CLI. See
[`mkosi/platform/x86/x86-rauc-boot-adapter.sh`](../mkosi/platform/x86/x86-rauc-boot-adapter.sh)
and
[`mkosi/platform/x86/x86-boot-state.sh`](../mkosi/platform/x86/x86-boot-state.sh)
...
[`mkosi/platform/x86/README.md`](../mkosi/platform/x86/README.md) before running
...
bats tests/x86-raucb-bundle.bats
```

`tests/x86-raucb-bundle.bats` asserts (see its header comment): (1) the
...
- [ ] `tests/x86-raucb-bundle.bats` passes (offline, §9.1 — run this first).
...
- [ ] `grubenv` inspection (§9.3) matches the expected `ORDER`/`_OK`/`_TRY` state at
      each step.
- [ ] `tests/realhw-suite.sh` (LIVE mode, `BOARD=x86-minipc`) exits 0 — note any
...
[`mkosi/platform/x86/README.md`](../mkosi/platform/x86/README.md) §2.0).
...
BUNDLE_TO_CHECK="images/x86-minipc/<ts>.raucb"   # replace with the actual bundle path
rauc info -C keyring:check-purpose=codesign --keyring=.dev-keys/dev-root-ca.pem "${BUNDLE_TO_CHECK}"
```
...
`manifests/families/x86_64.yaml` (`hw_accel_gstreamer_plugins`) against what is
...
| [`mkosi/platform/x86/README.md`](../mkosi/platform/x86/README.md) | x86 encode strategy (D1) + full A/B GRUB bootloader design |
| [`manifests/boards/x86-minipc.yaml`](../manifests/boards/x86-minipc.yaml) | x86-minipc board manifest |
| [`manifests/families/x86_64.yaml`](../manifests/families/x86_64.yaml) | x86_64 family manifest (encode packages, bootloader adapter, partition template) |
| [`tests/x86-raucb-bundle.bats`](../tests/x86-raucb-bundle.bats) | Offline signed-bundle acceptance gate for the x86 OTA path |
| [`tests/qemu-x86.sh`](../tests/qemu-x86.sh) | Headless qemu boot smoke (offline proxy for "does it boot") |
| [`docs/DEFERRED.md`](DEFERRED.md) | Deferred-item index (RK3588-focused; x86 items tracked inline where applicable) |
