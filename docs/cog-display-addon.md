...
resolves against
(`mkosi/build/app/var/lib/apt/lists/deb.debian.org_debian_dists_bookworm_main_binary-arm64_Packages`),
...
```bash
# mkosi/app/cog.sysext.conf   (scaffold — wire up only after the HW gate clears)
SYSEXT_NAME=cog
...
#
#   source mkosi/app/sysext-build.lib.sh
...
- **Now baked into the base image (Platform layer).** The exact package
  ...
  `fetch_rk3588_userspace` in `lib/fetch-debs.sh` (excluded from the Armbian
...
`docs/cog-display-hw-checklist.md` (build+sign the real closure → stage →
...
| `mkosi/LAYER-MAP.md` | layer boundaries — why GPU userspace is Platform, apps are Layer 4 |
| `mkosi/app/sysext-build.lib.sh` | the extract → prune → squashfs builder this recipe reuses |
| `manifests/families/rk3588.yaml` | RK3588 BSP/firmware sources (now bakes `libmali-valhall-g610-g24p0-wayland-gbm`) |
| `manifests/rk3588-userspace-deb-versions.txt` | pinned + SHA-256-verified RK3588 GPU/MPP/RGA userspace .debs (incl. the libmali variant above) |
