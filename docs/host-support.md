...
([`lib/assemble-disk.sh`](../lib/assemble-disk.sh)) — `systemd-repart`,
...
([`ci/Dockerfile`](../ci/Dockerfile)) that every assembly step in
...
```bash
DOCKER_CONTEXT=default GITHUB_WORKSPACE="$PWD" ./ci/check-builder-resources.sh
```
...
`mkosi/build` and `mkosi/cache`. The pre-checkout pass recovers when a host
or runner interruption prevented the prior post-run pass. Do not broaden this
allowlist: `.staging`, `images`, `candidate`, source, and trust inputs are not
...
and `debian-archive-keyring`): `./build rock-5b-plus --native`.
...
into the container (`-v ${PIPELINE_DIR}:/work`). Under enforcing SELinux, Podman/Docker
...
docker run --rm -v "$PWD":/repo:ro debian:trixie-slim bash -euo pipefail -c '
  ...
  cp -r /repo/v2 /tmp/v2
  bash /tmp/lib/assemble-disk.sh verify            # sgdisk + repart geometry
  mkdir -p /tmp/t/etc && echo hi >/tmp/t/etc/hostname
  bash /tmp/lib/assemble-disk.sh build --output /tmp/x.img \
...
- [`ci/Dockerfile`](../ci/Dockerfile) — the pinned trixie builder (mkosi 26)
- [`mkosi/mkosi.conf`](../mkosi/mkosi.conf) (§28-29) — binfmt/qemu-user-static cross-arch note
- [`lib/assemble-disk.sh`](../lib/assemble-disk.sh) — the loop-free Stage-4 assembler
