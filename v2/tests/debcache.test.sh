...
# (v2/lib/fetch/debcache.sh) and its three fetch-family call sites.
#
...
DEBCACHE_SRC="${V2}/lib/fetch/debcache.sh"
...
if ! grep -q 'debcache_store "${destination}"' "${V2}/lib/fetch/pool.sh"; then
...
for module in bsp userspace firstparty; do
	if ! grep -q 'debcache_try_hit' "${V2}/lib/fetch/${module}.sh"; then
...
bsp_hits="$(grep -c 'debcache_try_hit' "${V2}/lib/fetch/bsp.sh")"
fp_hits="$(grep -c 'debcache_try_hit' "${V2}/lib/fetch/firstparty.sh")"
...
if command -v git >/dev/null 2>&1 && git -C "${REPO_ROOT}" rev-parse --git-dir >/dev/null 2>&1; then
	if ! git -C "${REPO_ROOT}" check-ignore -q "v2/mkosi/.staging/.debcache/x_1_arm64.deb"; then
