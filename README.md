...
| either RK3588 board | `edge-test` (debug sibling) | `./build <board> --variant edge-test` | KASAN + lockdep + the CeraLive MPP fault-injection seam; `ci/check-release-variant.sh` refuses to release it by property, and `[6c/9]` reports its size instead of enforcing the ceiling |
...
## HDMI-RX EDID Profiles

The SoC's HDMI receiver ships with no EDID of its own — its own Kconfig says a
product is expected to load a custom, branded one from userspace, and until
something does, an attached source reads nothing on the DDC lines and either
falls back to its most conservative mode or refuses to output at all.
`ceralive-hdmirx-edid.service` writes one at boot.

The image carries **two** committed 256-byte blobs and programs exactly one:

| Profile | Advertises | Use it when |
|---|---|---|
| `full` (default) | 4K60 at 594 MHz over RGB/4:2:2, an HDMI Forum VSDB with SCDC Present and a 600 MHz max TMDS character rate, plus a Y420CMDB marking the 4K60 modes as additionally 4:2:0-capable | normally — it is everything the receiver can take |
| `robust-4k60` | an HDMI 1.4-class posture: **no** HDMI Forum VSDB (so no SCDC and a 300 MHz ceiling), 4K60 offered **only** as 4:2:0 through a Y420VDB, and a 4K30/297 MHz preferred timing with 1080p60 second | a source cannot hold a clean 594 MHz link, and you would rather it fall back to safer signalling than fail |

Which one is active is one key in one file on the persistent partition:

```sh
# /data/ceralive/hdmirx.conf
hdmirx.edid_profile = robust-4k60
```

```sh
sudo systemctl restart ceralive-hdmirx-edid.service   # apply the change
journalctl -u ceralive-hdmirx-edid.service -b          # see which profile won
```

`/data` is the partition an A/B OTA preserves, so the choice survives an update.
An absent file, an absent key, an unreadable or implausibly large file, an
unrecognised value, or a selected profile whose blob is missing all resolve to
`full` and say so in the journal — a fresh or upgraded device therefore behaves
exactly as it did before profiles existed.

**Never mid-stream.** Writing an EDID drops HPD, writes, and raises HPD again,
which under a live capture renegotiates the source in the middle of a broadcast.
The kernel refuses the write with `-EBUSY` while the node is streaming and the
service honours that: it refuses without retrying and without forcing, and fails
loudly rather than reporting an EDID it did not apply. Stop the stream first.

Both blobs are generated by `tools/gen-hdmirx-edid.py` (`--profile`,
`--list-profiles`) and committed alongside their `edid-decode` output for human
review. CI regenerates both, byte-compares them, checks each `.txt` really
