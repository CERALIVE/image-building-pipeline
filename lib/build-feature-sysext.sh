...
#                         and test builds it defaults to .dev-addon-keys/gnupg,
#                         a THROWAWAY keypair generated on first use (gitignored,
#                         exactly like the RAUC dev keypair in .dev-keys). It is
...
# Throwaway DEV add-on signing home. SEPARATE from .dev-keys (RAUC) — distinct
# trust domain. Generated on first use, gitignored. CI overrides via --keyring.
DEV_ADDON_KEYS_DIR="${CERALIVE_ADDON_KEYS_DIR:-${PIPELINE_DIR}/.dev-addon-keys}"
...
Env:
  CERALIVE_ADDON_KEYS_DIR  override the dev keys dir (default .dev-addon-keys).
...
assert_descriptor_valid() {
  ...
  python3 "${PIPELINE_DIR}/ci/validate-manifests.py" --file "${descriptor}" \
...
build-feature-sysext() {
  ...
  descriptor="${descriptor:-${PIPELINE_DIR}/manifests/addons/${feature}.json}"
