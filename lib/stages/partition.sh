...
stage_partition() {
  ...
  cp "${PIPELINE_DIR}/manifests/rk3588-bluetooth-firmware-roots.txt" "${staging}/rk3588-bluetooth-firmware-roots.txt"
  ...
  local firstparty_names=" libsrt1.5-ceralive cerastream gstreamer1.0-libuvcsrc ceralive-device srtla modemmanager libmm-glib0 libmbim-glib4 libmbim-proxy libmbim-utils libqmi-glib5 libqmi-proxy libqmi-utils libqrtr-glib0 ceralive-modem-support "
  local deb pkg
  shopt -s nullglob
