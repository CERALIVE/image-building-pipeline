...
# never auto-built; the default baked tag is built from ci/Dockerfile when not
...
stage_mkosi() {
  ...
  build_version="$(git -C "${PIPELINE_DIR}" rev-parse --short HEAD 2>/dev/null || true)"
...
mkosi_invoke() {
  ...
  "${runtime}" run --rm --privileged \
    ...
    bash -euo pipefail -c '
      command -v mkosi >/dev/null 2>&1 || {
        echo "FATAL: builder image lacks mkosi — an overridden MKOSI_BUILDER_IMAGE must bake mkosi '"${MKOSI_VERSION_PIN}"' (see ci/Dockerfile)" >&2
        exit 1
      }
      if [[ -n "${APT_GPG_PUBLIC_B64:-}" ]]; then
