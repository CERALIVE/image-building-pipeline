...
image-building-pipeline/dev-sync --all
```
...
```bash
DRY_RUN=1 image-building-pipeline/dev-sync --all
```

Watch only what you're changing:

```bash
image-building-pipeline/dev-sync --frontend          # UI changes only
image-building-pipeline/dev-sync --backend           # backend binary only
image-building-pipeline/dev-sync --frontend --backend  # both, not native
```
...
```bash
ssh root@<selected-hostname>.local bash < image-building-pipeline/lib/dev-sync/setup.sh
```
...
```bash
cp image-building-pipeline/lib/dev-sync/.dev-sync.yaml.example .dev-sync.yaml
```
...
3. `image-building-pipeline/lib/dev-sync/.dev-sync.yaml` (alongside the scripts)
...
```bash
bash image-building-pipeline/lib/dev-sync/config.sh
```
...
DEV_SYNC_NATIVE_ARGS="--from-deb /path/to/debs" image-building-pipeline/dev-sync --native

# From an extracted staging tree:
DEV_SYNC_NATIVE_ARGS="--staging /path/to/staging" image-building-pipeline/dev-sync --native

# From a prebuilt .raw:
DEV_SYNC_NATIVE_ARGS="--raw /path/to/srtla.raw" image-building-pipeline/dev-sync --native
```
...
image-building-pipeline/dev-sync --backend --force

# Queue the sync until the stream stops, then proceed automatically:
image-building-pipeline/dev-sync --backend --defer
```
...
```bash
DEV_SYNC_TARGET_IP=192.168.1.50 image-building-pipeline/dev-sync --all
```

To skip mDNS entirely and always use the IP:

```bash
DEV_SYNC_TARGET_HOST="" DEV_SYNC_TARGET_IP=192.168.1.50 image-building-pipeline/dev-sync --all
```
...
2. Cross-compile (backend only): `DEV_SYNC_ALLOW_CROSS_BUILD=1 image-building-pipeline/dev-sync --backend`. This uses `bun --target=bun-linux-arm64`.
...
```bash
ssh root@<selected-hostname>.local bash < image-building-pipeline/lib/dev-sync/setup.sh --dry-run
```
