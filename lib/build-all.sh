...
#
# `build --all` / `build --only b1,b2` (build) resolve a multi-board selection
...
BOARDS_DIR="${BOARDS_DIR:-${PIPELINE_DIR}/manifests/boards}"
LOGS_DIR="${LOGS_DIR:-${PIPELINE_DIR}/${CERALIVE_REL_LOGS_DIR}}"
...
# (first matching extension wins). Mirrors build::find_manifest; kept local so
