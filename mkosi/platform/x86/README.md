...
```
mkosi/platform/x86/test-x86-fallback.sh   # 72 assertions, no GRUB/qemu/root
tests/qemu-x86.sh --fallback-selftest     # forced-primary-failure rollback proof
```
...
(`tests/manifest.bats` → `run-tests`) so CI gates on it.
