...
keypair** in `.dev-addon-keys/` (gitignored), exactly mirroring how
`../rauc/ceralive-keyring.pem` is the dev RAUC root. It lets local/dev images
verify locally-built add-ons out of the box.

- **Local/dev**: `lib/build-feature-sysext.sh` signs with `.dev-addon-keys/`
  (auto-generated on first use). Re-export this file after regenerating that
  keypair: `gpg --homedir .dev-addon-keys/gnupg --export > addon-keyring.gpg`.
