...
# docs/deferred-ceraui-sysext.md. Until then, appfs is the only viable backend.
#
...
# The app-layer contract lives in lib/app-layer/ (this file is mkosi/app/).
...
assert_full_filesystem_payload() {
  ...
  if [[ "${#missing[@]}" -ne 0 ]]; then
    die "ceraui appfs payload incomplete — missing: ${missing[*]} (CeraUI .deb layout changed? re-check docs/deferred-ceraui-sysext.md)"
  fi
