...
#
# THREE ARTIFACTS, NOT TWO — and the third is the reason this block differs from
# its four siblings above. Besides the script and the unit, this one installs a
# committed BINARY: mkosi/runtime/edid/ceralive-hdmirx.edid, generated byte-exact
# by tools/gen-hdmirx-edid.py and gated in CI against generator drift. It lands
# at /etc/ceralive/hdmirx.edid because /etc/ceralive is where this image already
# installs read-only runtime data (ingest-firewall.nft, uart-bootstrap-public.pem)
# and because postinst.d/persistence.sh's /etc/ceralive -> /data relocation loop
# is a NAMED list, so a new file there is not swept onto /data. That path is the
# script's own CERALIVE_HDMIRX_EDID default: producer and consumer must agree.
#
...
setup_hdmirx_edid() {
  log "installing the CeraLive product EDID for the HDMI-RX receiver (ceralive-hdmirx-edid.service — the receiver ships with NO EDID and is not functional until one is written)"
  local src="${CERALIVE_RUNTIME_SRC:-}"
  ...
  [[ -s "${src}/edid/ceralive-hdmirx.edid" ]] \
    || die "hdmirx EDID blob not found: ${src}/edid/ceralive-hdmirx.edid (is \$SRCDIR/runtime mounted?)"
  ...
  install -D -m 0644 "${src}/edid/ceralive-hdmirx.edid" "${blob_dir}/hdmirx.edid"
