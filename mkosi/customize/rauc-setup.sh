...
# mkosi/runtime/rauc/ceralive-keyring.pem. It reaches this chroot module the
...
# Source artifacts (committed): mkosi/runtime/rauc/{ceralive-keyring.pem,system.conf}
...
install_system_conf_fallback() {
  ...
  if [[ -n "${RAUC_SRC_DIR}" && -s "${RAUC_SRC_DIR}/system.conf" ]]; then
    ...
    # mkosi/runtime/rauc/system.conf and install-boot.sh.
