...
# the orchestrator's [3/9] step (lib/stages/partition.sh) then partitions each staged .deb into BSP vs
...
ORCH="${PIPELINE_DIR}/lib/stages/partition.sh"
...
if (( ${#missing[@]} > 0 )); then
  fail "stages/partition.sh firstparty_names is missing $(( ${#missing[@]} )) fetched first-party package(s): ${missing[*]} — a real build would die with 'unclassified staged package'"
fi
