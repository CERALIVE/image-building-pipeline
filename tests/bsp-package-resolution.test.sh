...
AUTH="${PIPELINE_DIR}/lib/fetch-debs-auth.sh"
FETCH="${PIPELINE_DIR}/lib/fetch-debs.sh"
PINS="${PIPELINE_DIR}/manifests/armbian-bsp-deb-versions.txt"
TMP="$(mktemp -d)"
trap 'rm -rf "${TMP}"' EXIT
...
non_armbian_plan="$(fetch_bsp \
  "${PIPELINE_DIR}/manifests/families/x86_64.yaml" "${TMP}/non-armbian" 2>&1)"
...
DRY_RUN=""
if (fetch_bsp "${PIPELINE_DIR}/manifests/families/x86_64.yaml" \
