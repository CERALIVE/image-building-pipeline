...
setup() {
  ...
  VERIFY="$PIPELINE_DIR/lib/verify-disk.sh"
  ...
  cat "$POSTINST_ENTRY" "$PIPELINE_DIR"/mkosi/customize/postinst.d/*.sh >"$POSTINST_SRC"
}
...
# staging root (ROOT=), exactly as mkosi/mkosi.images/platform/mkosi.finalize
...
@test "resolve_partlabel is the single label resolver on both the host and chroot tracks" {
  local probe
  for probe in "$PIPELINE_DIR/lib/common.sh" "$PIPELINE_DIR/mkosi/customize/postinst-lib.sh"; do
...
@test "both RAUC fallback system.conf tracks resolve their slot labels" {
  local track
  for track in "$PIPELINE_DIR/mkosi/customize/rauc-setup.sh" \
...
@test "CERALIVE_BENCH_LABELS reaches every subimage chroot (env_names + PassEnvironment)" {
  run grep -Fq 'CERALIVE_BENCH_LABELS' "$PIPELINE_DIR/lib/orchestrate.sh"
  [ "$status" -eq 0 ]
  run grep -Eq '^PassEnvironment=.*CERALIVE_BENCH_LABELS' "$PIPELINE_DIR/mkosi/mkosi.conf"
  [ "$status" -eq 0 ]
}
