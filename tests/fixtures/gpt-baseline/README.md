...
Captured from `lib/assemble-disk.sh` **before** the `CERALIVE_BENCH_LABELS`
bench-PARTLABEL overlay existed, at commit `1af9116`:

```bash
lib/assemble-disk.sh build --output ab.img --total-mb 10513 --no-format
lib/assemble-disk.sh build --output ss.img --total-mb  8192 --no-format --single-slot
```
...
role as `tests/manifests/fixtures/vendor-baseline/*.params` for the `--variant`
mechanism.
...
Consumed by `tests/bench-partlabels.bats`.
