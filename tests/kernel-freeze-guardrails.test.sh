...
LIB="${PIPELINE_DIR}/mkosi/customize/postinst-lib.sh"
...
grep -qE '^  freeze_boot_packages( |$)' "${POSTINST}" \
  || fail "the runtime executor's main() no longer calls freeze_boot_packages — the guardrails would never ship (run-all.sh's runtime modules are NOT run by ./build)"
