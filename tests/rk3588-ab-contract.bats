...
setup() {
  ...
  ASSEMBLE="$PIPELINE_DIR/lib/assemble-disk.sh"
  RESOLVE="$PIPELINE_DIR/lib/resolve.sh"
  PREFLASH="$TESTS_DIR/preflash-verify.sh"
  BOOT_DIR="$PIPELINE_DIR/mkosi/platform/boot"
}
...
write_small_repart_defs() {
  local defs="$1"
  mkdir -p "$defs"
  cp "$PIPELINE_DIR/mkosi/repart/10-boot.conf" "$defs/10-boot.conf"
...
make_rootfs_tree() {
  ...
  cp "$PIPELINE_DIR/.dev-keys/dev-root-ca.pem" "$tree/etc/rauc/ceralive-keyring.pem"
...
EOF
    ...
    COMPATIBLE_STRING=ceralive-rock-5b-plus BUNDLE_VERSION=contract \
      BUNDLE_OUT_DIR="$base" BUNDLE_TS=update CERALIVE_RAUC_PKI_DIR="$PIPELINE_DIR/.dev-keys" \
      REPRODUCIBLE=1 bash "$PIPELINE_DIR/lib/build-bundle.sh" rock-5b-plus "$base/rootfs" >/dev/null
    cp "$PIPELINE_DIR/.dev-keys/dev-root-ca.pem" "$base/keyring.pem"
...
build_missing_artifact_image() {
  ...
  if [[ "$artifact" == etc/rauc/ceralive-keyring.pem ]]; then
    cp "$PIPELINE_DIR/mkosi/runtime/rauc/ceralive-keyring.pem" "$tree/$artifact"
  fi
...
@test "RK3588 boot paths identify the booted slot explicitly to RAUC" {
  ...
  [ "$status" -eq 0 ]
  run bash "$PIPELINE_DIR/tests/recovery-script-contract.test.sh"
  [ "$status" -eq 0 ]
}
...
@test "blocking v2 gate includes boot fallback rollback real RAUC and preflash negatives" {
  run bash "$PIPELINE_DIR/tests/production-ci-scope-contract.test.sh"
  [ "$status" -eq 0 ]
}
