...
#
# Usage:  tests/package-migration-coverage.sh [evidence-file]
...
PKGDIR="${PIPELINE_DIR}/manifests/packages"
FAMDIR="${PIPELINE_DIR}/manifests/families"
...
RUNTIME_DIR="${PIPELINE_DIR}/mkosi/runtime"
...
if [[ ! -f "${BASECONF}" && ! -f "${CIMG}" ]]; then
  echo "SKIP: legacy package sources retired (T24) — MIGRATE complete; ongoing guard is lib/parity-check.sh."
  exit 0
fi
...
mkdir -p "$(dirname "${EVIDENCE}")"
{
  ...
  printf '%-34s %s\n' "LEGACY PACKAGE" "PIPELINE_DIR HOME"
