...
R2_PUBLISHER="${REPO}/ci/publish-immutable-r2-pair.sh"
...
for required in (
    ...
    'git show "${merge_sha}:ci/publish-immutable-r2-pair.sh"',
):
...
for cleanup_index in (pre_cleanup_index, post_cleanup_index):
    ...
    # The deleted set comes from the single CERALIVE_GENERATED_CLEANUP_PATHS
    # declaration, expanded element by element with a literal-path guard, so a
    # widened or globbed entry fails the step instead of the workspace.
    assert cleanup["env"]["CLEANUP_PATHS"] == "${{ env.CERALIVE_GENERATED_CLEANUP_PATHS }}"
    assert 'targets+=("/work/${rel}")' in cleanup_run
    assert "refusing to delete non-literal path" in cleanup_run
    assert 'rm -rf -- "${targets[@]}"' in cleanup_run
    assert cleanup_run.count("rm -rf") == 1
    assert "chown" not in cleanup_run and "find " not in cleanup_run
    assert "rm -rf -- /work" not in cleanup_run
assert "if" not in steps[pre_cleanup_index]
assert steps[post_cleanup_index]["if"] == "${{ always() }}"

resource_step = steps[resource_index]
assert resource_step.get("run") == "./ci/check-builder-resources.sh", (
...
assert builder_with["context"] == "ci"
assert builder_with["file"] == "ci/Dockerfile"
...
assert "hashFiles('.mkosi-version')" in cache_meta_env["MKOSI_TOOL_KEY"]
assert "hashFiles('build'" in cache_meta_env["MKOSI_SOURCE_KEY"]
