...
# (expected to differ): [...]".
#
# Run:  tests/parametric.sh            # rock-5b-plus  vs  x86-minipc
#       BOARD_ARM=orange-pi-5-plus tests/parametric.sh
#
...
MKOSI_IMAGES="${PIPELINE_DIR}/mkosi/mkosi.images"
PKG_DIR="${PIPELINE_DIR}/manifests/packages"
...
check_shared_files() {
  ...
  local cust="${PIPELINE_DIR}/mkosi/customize"
  if [[ -d "${cust}" ]]; then
    local n
